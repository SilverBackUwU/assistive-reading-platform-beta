...
- Do NOT include markdown image blocks
- Do NOT include image data, data:image URIs, or base64
- Return ONLY clean plain extracted text"""
...
class GeminiOCR(OCREngine):
        ...
        try:
            ...
            if contains_encoded_image_payload(raw_text) or contains_giant_encoded_string(raw_text):
                return LayerOutput(
                    ...
                    confidence=0.0,
                    success=False,
                    error="Gemini returned encoded image data instead of OCR text",
                )
            ...
            if not text:
                return LayerOutput(
                    ...
                    confidence=0.0,
                    success=False,
                    error="Gemini returned empty or corrupted text",
                )
    ...
    async def arbitrate(
        ...
        try:
            ...
            raw_text = response.text or "[]"
            logger.debug("Gemini arbiter raw response preview: %s", preview_ocr_text(raw_text))
            raw = raw_text.strip()
